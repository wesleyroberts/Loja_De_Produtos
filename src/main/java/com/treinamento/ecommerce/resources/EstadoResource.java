...
public class EstadoResource {
    ...
    @GetMapping("/all")
    public ResponseEntity<List<Estado>>findAllEstado(){
    ...
    public ResponseEntity<Page<Estado>> findPage(
        ...
        Page<Estado> list = estadoService.findPage(page, size, orderBy, direction);
        return ResponseEntity.status(HttpStatus.FOUND).body(list);
    }

    @PostMapping("/create")
    public ResponseEntity<Void> create(@Valid @RequestBody EstadoDTO estadoDTO){
